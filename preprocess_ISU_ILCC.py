...
def preprocess_isu_ilcc_data(raw_data_path="22582234.zip"):
    """
    处理ISU-ILCC数据集，将原始数据转换为标准格式
    
    Args:
        raw_data_path: 原始数据文件路径，默认为当前目录下的22582234.zip
    """
    raw_file = Path(raw_data_path)
    ...
    # 解压主文件
    extract_dir = raw_file.parent / '22582234'
    if not extract_dir.exists():
        print("正在解压主数据文件...")
        with zipfile.ZipFile(raw_file, 'r') as zip_ref:
            for file in tqdm(zip_ref.namelist(), desc="解压主文件"):
                zip_ref.extract(file, extract_dir)
    else:
        print("主数据文件已解压，跳过...")
    
    # 解压循环数据和RPT数据
    cycle_zip_path = extract_dir / 'Cycling_json.zip'
    rpt_zip_path = extract_dir / 'RPT_json.zip'
    
    if not (extract_dir / 'Cycling_json').exists():
        print("正在解压循环数据文件...")
        with zipfile.ZipFile(cycle_zip_path, 'r') as zip_ref:
            for file in tqdm(zip_ref.namelist(), desc="解压循环文件"):
                zip_ref.extract(file, extract_dir)
    else:
        print("循环数据文件已解压，跳过...")
    
    if not (extract_dir / 'RPT_json').exists():
        print("正在解压RPT数据文件...")
        with zipfile.ZipFile(rpt_zip_path, 'r') as zip_ref:
            for file in tqdm(zip_ref.namelist(), desc="解压RPT文件"):
                zip_ref.extract(file, extract_dir)
    else:
        print("RPT数据文件已解压，跳过...")
    
    # 读取有效电池列表
    valid_cells_file = extract_dir / 'Valid_cells.csv'
    if not valid_cells_file.exists():
        print(f"错误: 找不到有效电池列表文件 {valid_cells_file}")
        return
    
    valid_cells = pd.read_csv(valid_cells_file).values.flatten().tolist()
    batch2 = ['G57C1', 'G57C2', 'G57C3', 'G57C4', 'G58C1', 'G26C3', 'G49C1', 'G49C2', 'G49C3', 'G49C4', 'G50C1',
              'G50C3', 'G50C4']
    
    # 创建输出目录
    output_dir = Path('data/ISU_ILCC')
    output_dir.mkdir(parents=True, exist_ok=True)
    
    processed_count = 0
    skipped_count = 0
    
    for cell in tqdm(valid_cells, desc='处理ISU_ILCC电池'):
        print(f'正在处理电池 {cell}')
        ...
        # 检查是否已处理
        output_file = output_dir / f'ISU-ILCC_{cell}.pkl'
        if output_file.exists():
            print(f"电池 {cell} 已处理，跳过...")
            skipped_count += 1
            continue
        ...
        # 处理单个电池数据
        battery_data = process_single_battery(extract_dir, cell, subfolder)
        ...
        processed_count += 1
        print(f'电池 {cell} 处理完成，保存到 {output_file}')
    
    print(f"\n处理完成!")
    print(f"成功处理: {processed_count} 个电池")
    print(f"跳过: {skipped_count} 个电池")

def process_single_battery(zip_path, cell, subfolder):
    """处理单个电池的数据"""
    
    # 获取循环数据
    cycling_dict = load_cycling_data(zip_path, cell, subfolder)
    
    # 转换为DataFrame格式
    df = pd.DataFrame()
    
    for index in range(len(cycling_dict['QV_charge']['I'])):
        cycle_df = pd.DataFrame()
        
        # 合并充电和放电数据
        charge_len = len(cycling_dict['QV_charge']['I'][index])
        discharge_len = len(cycling_dict['QV_discharge']['I'][index])
        total_len = charge_len + discharge_len
        
        cycle_df['cycle_number'] = [index + 1] * total_len
        
        # 电流数据
        cycle_df['I'] = (cycling_dict['QV_charge']['I'][index] + 
                        cycling_dict['QV_discharge']['I'][index])
        
        # 电压数据
        cycle_df['V'] = (cycling_dict['QV_charge']['V'][index] + 
                        cycling_dict['QV_discharge']['V'][index])
        
        # 时间数据
        cycle_df['t'] = (cycling_dict['QV_charge']['t'][index] + 
                        cycling_dict['QV_discharge']['t'][index])
        
        # 容量数据
        charge_q = cycling_dict['QV_charge']['Q'][index]
        discharge_q = cycling_dict['QV_discharge']['Q'][index]
        
        cycle_df['Q_charge'] = charge_q + [0] * discharge_len
        cycle_df['Q_discharge'] = [0] * charge_len + discharge_q
        
        df = pd.concat([df, cycle_df], ignore_index=True)
    
    # 清理RPT测试数据
    df = clean_rpt_data(df, zip_path, cell, subfolder)
    
    # 组织电池数据
    battery_data = organize_battery_data(df, f'ISU-ILCC_{cell}')
    
    return battery_data

def load_cycling_data(zip_path, cell, subfolder):
    """加载循环数据"""
    cycling_file = zip_path / f'Cycling_json/{subfolder}/{cell}.json'
    
    with open(cycling_file, 'r') as file:
        data_dict = json.loads(json.load(file))
...
def load_rpt_data(zip_path, cell, subfolder):
    """加载RPT数据"""
    rpt_file = zip_path / f'RPT_json/{subfolder}/{cell}.json'
...
def clean_rpt_data(df, zip_path, cell, subfolder):
    """清理RPT测试数据"""
    should_exclude = []
    data_dict_cycle = load_cycling_data(zip_path, cell, subfolder)
    data_dict_rpt = load_rpt_data(zip_path, cell, subfolder)
    cycle_start = data_dict_cycle['QV_discharge']['t']
    ...
    # 重新编号周期
    cycle_numbers = sorted(set(df['cycle_number'].tolist()))
    for old_num, new_num in zip(cycle_numbers, range(1, len(cycle_numbers) + 1)):
        df.loc[df['cycle_number'] == old_num, 'cycle_number'] = new_num
    
    return df

def organize_battery_data(timeseries_df, name):
    """组织电池数据为标准格式"""
    timeseries_df = timeseries_df.sort_values('t')
    cycle_data = []
    ...
    # 计算SOC范围
    charge_start_soc, discharge_end_soc = calculate_soc_range(timeseries_df, name)
    
    # 获取充放电倍率
    rates = get_cycling_rates(name)
    ...
    battery_data = {
        ...
        'anode_material': 'graphite',
        'cathode_material': 'NMC',
        'discharge_protocol': [{'rate_in_C': float(rates[1]), 'start_soc': 1.0, 'end_soc': discharge_end_soc}],
        'charge_protocol': [{'rate_in_C': float(rates[0]), 'start_soc': charge_start_soc, 'end_soc': 1.0}],
        'nominal_capacity_in_Ah': 0.25,
        'min_voltage_limit_in_V': 3.0,
        'max_voltage_limit_in_V': 4.2,
        'SOC_interval': [charge_start_soc, 1]
    }
...
def calculate_soc_range(df, name, nominal_capacity=0.25):
    """计算SOC范围"""
    charge_capacity = df.loc[df['cycle_number'] == 1, 'Q_charge'].max()
    soc_charge_interval = charge_capacity / nominal_capacity
    if soc_charge_interval > 1:
        soc_charge_interval = 1
    charge_start_soc = 1 - soc_charge_interval
    
    discharge_capacity = df.loc[df['cycle_number'] == 1, 'Q_discharge'].max()
    soc_discharge_interval = discharge_capacity / nominal_capacity
    if soc_discharge_interval > 1:
        soc_discharge_interval = 0
    discharge_end_soc = 1 - soc_discharge_interval
    
    return charge_start_soc, discharge_end_soc

def get_cycling_rates(name):
    """获取充放电倍率"""
    cycling_rates = {
    ...
    return cycling_rates.get(name, [1.0, 1.0])  # 默认值

# 主函数
if __name__ == "__main__":
    print("开始处理ISU-ILCC数据集...")
    preprocess_isu_ilcc_data()
    print("数据处理完成!")